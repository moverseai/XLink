...
xLinkPlatformErrorCode_t XLinkPlatformFindDevicesDynamic(
    const deviceDesc_t in_deviceRequirements, deviceDesc_t* out_foundDevices,
    unsigned sizeFoundDevices, unsigned int* out_amountOfFoundDevices,
    int timeoutMs, bool (*cb)(deviceDesc_t*, unsigned int)) {
  ...
  *out_amountOfFoundDevices = 0;
  ...
  if (XLinkIsProtocolInitialized(X_LINK_TCP_IP) &&
      tcpip_create_search_context(&tcpip_ctx, in_deviceRequirements) ==
  ...
  do {
    ...
    switch (in_deviceRequirements.protocol) {
      ...
      case X_LINK_USB_VSC:
        if (!usb_vsc_available) {
          return static_cast<xLinkPlatformErrorCode_t>(
              (int)X_LINK_PLATFORM_DRIVER_NOT_LOADED +
              in_deviceRequirements.protocol);
        }
        // Check if protocol is initialized
        getUSBDevices(in_deviceRequirements, pFoundDevices, sizeFoundDevices,
                      out_amountOfFoundDevices);

        /* TODO(themarpe) - reenable PCIe
        case X_LINK_PCIE:
            return getPCIeDeviceName(0, state, in_deviceRequirements,
        out_foundDevice);
        */
      ...
      case X_LINK_TCP_IP:
        ...
        return tcpip_perform_search(tcpip_ctx, pFoundDevices, sizeFoundDevices,
                                    out_amountOfFoundDevices);
      ...
      case X_LINK_ANY_PROTOCOL:
        ...
        if(XLinkIsProtocolInitialized(X_LINK_PCIE)) {
            numFoundDevices = 0;
            PCIe_rc = getPCIeDeviceName(0, state, in_deviceRequirements,
        out_foundDevice);
            // Found enough devices, return
            pFoundDevices += numFoundDevices;
            if (numFoundDevices >= sizeFoundDevices) {
        ...
        */
        ...
        if (tcpip_available) {
          numFoundDevices = 0;
          tcpip_perform_search(tcpip_ctx, pFoundDevices, sizeFoundDevices,
                               &numFoundDevices);
          *out_amountOfFoundDevices += numFoundDevices;
          pFoundDevices += numFoundDevices;
          sizeFoundDevices -= numFoundDevices;
    ...
    // Filter out duplicates - routing table will decide through which interface
    // the packets will traverse
    // TODO(themarpe) - properly separate interfaces.
    // Either bind to interface addr, or SO_BINDTODEVICE Linux, IP_BOUND_IF
    // macOS, and prefix interface name
    {
      ...
      for (int i = 0; i < *out_amountOfFoundDevices; i++) {
        ...
        for (int j = i - 1; j >= 0; j--) {
          // Check if duplicate
          if (devices[i].protocol == devices[j].protocol &&
              strcmp(devices[i].name, devices[j].name) == 0 &&
              strcmp(devices[i].mxid, devices[j].mxid) == 0) {
            duplicate = true;
            break;
          }
        }
